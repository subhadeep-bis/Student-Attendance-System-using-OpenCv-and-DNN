# Step1: Extract Embeddings from face dataset
...
ap.add_argument("-e", "--embeddings", required=True, help="path to out the face embeddings")
ap.add_argument("-d", "--detector", required=True, help="path to openCV face detector")
...
# load our serialized face detetor  from disk
...
for(i, imagePath) in enumerate(imagePaths):
	...
	if len(detections) > 0:
		# we're making an assumption that each image has only ONE face, so find the bounding box with the largest probability
		...
		if confidence > args["confidence"]:
			# compute the (x,y)-coordinate of the bounding box for the face
			...
			# construct the blob for face ROI, then pass the blob through our face embedding model to obtain 128-d
